pack;clc;clear all;close all; format long
...
% THIS MATLAB FUNCTION IS A TRANSLATION OF THE MarsConvertor.ipynd written
% by G. Sainton for ASPIC program
% @ A. LUCAS, 18 Dec. 2018. version 1.0 - Original from MarsConvertor.ipynd
...
origindate  = datetime(2018, 11, 26,  5, 10, 50.3356, 'TimeZone','Etc/GMT'); 
longitude   = 224.03;
...
disp(['j2000_ott= ',num2str(j2000_ott)])
%MTC = 24 * (((j2000_ott - 2451549.5)/ 1.0274912517) + KNORM - K);
...
function marsDate=utc2lmst(utc_date)
% ﻿
...
toto = delta_sec / (SECOND_IN_DAY*SOL_RATIO)
martianSol = int16(fix(delta_sec / (SECOND_IN_DAY*SOL_RATIO)))

MLST = Local_Mean_Solar_Time(longitude, jd_ott);
ihour = floor(MLST);
minutes = (MLST - ihour)*60.;
iminutes = floor(minutes);
secs = (minutes - iminutes)*60.;
iseconds = floor(secs);
milliseconds = floor((secs-iseconds)*1000);
 
%times = ihour*3600 + iminutes*60 + iseconds + milliseconds/1000;
