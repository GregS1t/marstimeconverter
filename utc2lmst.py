...
def main(argv):
    ...
    try:
        ...
        print('Usage: python utc2lmst.py -d <date> -f <format>')
        sys.exit(2)
    
    for opt, arg in opts:
        if opt == '-h':
            print("python utc2lmst.py -d <date> -f <format>\n"\
                  "       Function to convert UTC time to LMST time.\n\n"\
    ...
    if t_opt:
        if t_opt.lower() == "now":
            print(mDate.utc2lmst(output=output))
        else:
            try:
                t_utc = datetime.fromisoformat(t_opt)
                print(mDate.utc2lmst(utc_date=t_utc, output=output))
            except ValueError:
